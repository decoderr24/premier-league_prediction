...
# ================================
# 3️⃣ FEATURE ENGINEERING TINGKAT LANJUT: "TEAM FORM" (VERSI PERBAIKAN)
# ================================
print("\n🚀 Membuat fitur 'Team Form' (Performa Terkini)...")

team_stats = []
for index, row in hist_df.iterrows():
    team_stats.append({'Date': row['Date'], 'Team': row['Home'], 'GoalsFor': row['HomeGoals'], 'GoalsAgainst': row['AwayGoals'], 'ShotsFor': row['HomeShots'], 'SOT_For': row['HomeShotsOnTarget']})
    team_stats.append({'Date': row['Date'], 'Team': row['Away'], 'GoalsFor': row['AwayGoals'], 'GoalsAgainst': row['HomeGoals'], 'ShotsFor': row['AwayShots'], 'SOT_For': row['AwayShotsOnTarget']})
...
# Kita akan merge dua kali dengan cara yang lebih aman untuk menghindari nama kolom yang bentrok
# Merge untuk Tim Tuan Rumah
hist_df = pd.merge(
    hist_df,
    rolling_stats,
    left_on=['Date', 'Home'],
    right_on=['Date', 'Team'],
    how='left',
    suffixes=('', '_Home') # Menambahkan suffix jika ada nama kolom yg sama
).rename(columns={
    'AvgGoalsFor_L5': 'AvgGoalsFor_Home_L5',
    'AvgGoalsAgainst_L5': 'AvgGoalsAgainst_Home_L5',
    'AvgShotsFor_L5': 'AvgShotsFor_Home_L5',
    'AvgSOT_For_L5': 'AvgSOT_For_Home_L5'
}).drop('Team', axis=1)

# Merge untuk Tim Tamu
hist_df = pd.merge(
    hist_df,
    rolling_stats,
    left_on=['Date', 'Away'],
    ...
    'AvgGoalsFor_L5': 'AvgGoalsFor_Away_L5',
    'AvgGoalsAgainst_L5': 'AvgGoalsAgainst_Away_L5',
    'AvgShotsFor_L5': 'AvgShotsFor_Away_L5',
    'AvgSOT_For_L5': 'AvgSOT_For_Away_L5'
}).drop('Team', axis=1)
# ================================
...
h2h_results = []
# Menggunakan .index untuk memastikan urutan benar setelah merge
...
def predict_match(home_team_input, away_team_input, historical_df, model):
    ...
    if home_form.empty or away_form.empty:
        print(f"   ⚠️ Peringatan: Data form tidak ditemukan untuk salah satu tim.")
        return
    form_values = [

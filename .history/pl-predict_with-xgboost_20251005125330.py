...
try:
    ...
    # === PERBAIKAN: Membersihkan dan Memvalidasi Tipe Data ===
    stats_cols = ['HomeGoals', 'AwayGoals', 'HomeShots', 'AwayShots', 'HomeShotsOnTarget', 'AwayShotsOnTarget',
                  'HomeCorners', 'AwayCorners', 'HomeFouls', 'AwayFouls', 'HomeYellowCards', 'AwayYellowCards']
    for col in stats_cols:
        hist_df[col] = pd.to_numeric(hist_df[col], errors='coerce')
    
    hist_df.dropna(subset=stats_cols, inplace=True)
    print("✅ Data statistik berhasil divalidasi dan dibersihkan.")
    # =======================================================

    hist_df = hist_df.sort_values(by='Date').reset_index(drop=True)
    print("✅ Nama kolom dan format tanggal berhasil disesuaikan.")
except KeyError as e:
    print(f"❌ Error: Kolom {e} tidak ditemukan di 'epl-training.csv'."); exit()
...
def predict_match(home_team_input, away_team_input, historical_df, model, scaler):
    ...
    probs = model.predict_proba(input_features_scaled)[0]

import pandas as pd
import numpy as np
import xgboost as xgb # Menggunakan XGBoost
...
try:
    ...
    print(f"❌ Error: Kolom {e} tidak ditemukan di 'epl-training.csv'.")
    exit()

# ================================
# 3️⃣ FEATURE ENGINEERING TINGKAT LANJUT: "TEAM FORM" (DENGAN FITUR BARU)
# ================================
print("\n🚀 Membuat fitur 'Team Form' (Performa Terkini)...")
team_stats = []
for index, row in hist_df.iterrows():
    # Menambahkan data baru ke dalam list statistik
...
# Menghitung rolling average untuk semua fitur baru
rolling_stats = team_stats_df.groupby('Team').rolling(window=5, on='Date').mean().reset_index()
rolling_stats.rename(columns={
    'GoalsFor': 'AvgGoalsFor_L5', 'GoalsAgainst': 'AvgGoalsAgainst_L5', 'ShotsFor': 'AvgShotsFor_L5',
    'SOT_For': 'AvgSOT_For_L5', 'CornersFor': 'AvgCorners_L5', 'FoulsCommitted': 'AvgFouls_L5',
    'Yellows': 'AvgYellows_L5'
}, inplace=True)
rolling_stats = rolling_stats[['Team', 'Date', 'AvgGoalsFor_L5', 'AvgGoalsAgainst_L5', 'AvgShotsFor_L5', 'AvgSOT_For_L5', 'AvgCorners_L5', 'AvgFouls_L5', 'AvgYellows_L5']]

# Menggabungkan statistik form kembali ke dataframe utama
...
# ================================
# 5️⃣ Training & Evaluasi dengan MODEL BARU & FITUR BARU
# ================================
print("\n⚙️ Mempersiapkan data untuk training model...")
def get_result(row):
    if row["HomeGoals"] > row["AwayGoals"]: return 1
    elif row["HomeGoals"] < row["AwayGoals"]: return 0 # XGBoost mengharapkan label dari 0, 1, 2
    else: return 2 # Mengubah urutan: 0=Tamu Menang, 1=Tuan Rumah Menang, 2=Seri
hist_df["Result"] = hist_df.apply(get_result, axis=1)

# FITUR JAUH LEBIH BANYAK
features = [
    'AvgGoalsFor_Home_L5', 'AvgGoalsAgainst_Home_L5', 'AvgShotsFor_Home_L5', 'AvgSOT_For_Home_L5', 'AvgCorners_Home_L5', 'AvgFouls_Home_L5', 'AvgYellows_Home_L5',
    'AvgGoalsFor_Away_L5', 'AvgGoalsAgainst_Away_L5', 'AvgShotsFor_Away_L5', 'AvgSOT_For_Away_L5', 'AvgCorners_Away_L5', 'AvgFouls_Away_L5', 'AvgYellows_Away_L5',
...
# MENGGUNAKAN XGBOOST
model = xgb.XGBClassifier(n_estimators=200, random_state=42, objective='multi:softprob', eval_metric='mlogloss')
model.fit(X_train, y_train)
print("✅ Model berhasil dilatih.")

print("\n⚖️ Mengevaluasi akurasi model XGBoost...")
y_pred = model.predict(X_test)
acc = accuracy_score(y_test, y_pred)
print(f"🎯 Akurasi Model Secara Keseluruhan: {acc*100:.2f}%")
print("\nLaporan Detail Kinerja Model:\n")
# Sesuaikan target_names dengan urutan baru: 0=Tamu Menang, 1=Tuan Rumah Menang, 2=Seri
...
def predict_match(home_team_input, away_team_input, historical_df, model):
    ...
    form_values = [
        home_form['AvgGoalsFor_L5'].values[0], home_form['AvgGoalsAgainst_L5'].values[0], home_form['AvgShotsFor_L5'].values[0],
        home_form['AvgSOT_For_L5'].values[0], home_form['AvgCorners_L5'].values[0], home_form['AvgFouls_L5'].values[0], home_form['AvgYellows_L5'].values[0],
        away_form['AvgGoalsFor_L5'].values[0], away_form['AvgGoalsAgainst_L5'].values[0], away_form['AvgShotsFor_L5'].values[0],
        away_form['AvgSOT_For_L5'].values[0], away_form['AvgCorners_L5'].values[0], away_form['AvgFouls_L5'].values[0], away_form['AvgYellows_L5'].values[0]
    ]
    ...
    print(f"   - Form Gol (5 Laga): {home_team} ({form_values[0]:.2f}), {away_team} ({form_values[7]:.2f})")
    print(f"   - Form Corners (5 Laga): {home_team} ({form_values[4]:.2f}), {away_team} ({form_values[11]:.2f})")

    input_features = np.array([form_values + h2h_values])
    probs = model.predict_proba(input_features)[0]
    # Sesuaikan urutan probabilitas dengan urutan label XGBoost: 0=Tamu, 1=Tuan Rumah, 2=Seri

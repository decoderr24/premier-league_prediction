...
team_df[f'Roll_GoalsFor_L{WINDOW}'] = team_df.groupby('Team')['GoalsFor'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
team_df[f'Roll_GoalsAgainst_L{WINDOW}'] = team_df.groupby('Team')['GoalsAgainst'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
team_df[f'Roll_ShotsFor_L{WINDOW}'] = team_df.groupby('Team')['ShotsFor'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
team_df[f'Roll_WinRate_L{WINDOW}'] = team_df.groupby('Team')['Win'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
team_df[f'Roll_Points_L{WINDOW}'] = team_df.groupby('Team')['Points'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).sum())

# EWM (shift then ewm) => past-weighted recent form
team_df[f'EWM_GoalsFor_span{EWM_SPAN}'] = team_df.groupby('Team')['GoalsFor'].transform(lambda x: x.shift().ewm(span=EWM_SPAN, adjust=False).mean())
team_df[f'EWM_WinRate_span{EWM_SPAN}'] = team_df.groupby('Team')['Win'].transform(lambda x: x.shift().ewm(span=EWM_SPAN, adjust=False).mean())
...
home_matches[f'HomeWinRateLast{WINDOW}'] = home_matches.groupby('Team')['Win'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
# merge back
...
team_df[f'Roll_GoalDiff_L{WINDOW}'] = team_df.groupby('Team')['GoalDiff'].transform(lambda x: x.shift().rolling(WINDOW, min_periods=1).mean())
...
features = [
    ...
    f"Home_Roll_GoalsFor_L{WINDOW}".replace("Roll_", "")
]
...
print('📊 Evaluasi Akhir (Hold-out temporal):')
...
print('💾 Model tersimpan sebagai "model_epl_final_v4.joblib"')

# -------------------------
# 9) Fungsi predict_match (menggunakan model_features_v4.joblib untuk reindex)
# -------------------------
print('🔮 Menyediakan fungsi predict_match (v4) yang leakage-safe...')

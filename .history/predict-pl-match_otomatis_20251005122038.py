...
# ================================
# 1️⃣, 2️⃣, 3️⃣ (Memuat & Memproses Data) tidak berubah
# ================================
...
# Mengembalikan class_weight='balanced' karena biasanya memberikan hasil yang lebih baik
...
# === PERUBAHAN 2: Laporan akurasi detail diaktifkan kembali ===
...
def predict_match(home_team_input, away_team_input, passing_df, historical_df, model):
    ...
    # === PERUBAHAN 1: Peta nama tim disesuaikan dengan dataset Kaggle ===
    name_map = {
        # Input dari kita -> Nama di file epl-training.csv
        "Man Utd": "Man United",
        "Spurs": "Tottenham"
        # "Man City" dihapus dari sini, karena nama input dan nama di file sudah sama
    }
    ...
    def avg_pass_perc(team_name, passing_stats_df):
        # Kita gunakan nama standar dari data passing, karena sumbernya berbeda
        pass_name_map = {"Man Utd": "Manchester United", "Man City": "Manchester City", "Spurs": "Tottenham Hotspur"}
        pass_standard_name = pass_name_map.get(team_name, team_name)
        team_df = passing_stats_df[passing_stats_df["Squad"].str.contains(pass_standard_name, case=False, na=False)]
        if team_df.empty: return passing_stats_df["Total_Cmp%"].astype(float).mean()
        return team_df["Total_Cmp%"].astype(float).mean()

    home_pass = avg_pass_perc(home_team_input, passing_df) # Gunakan input asli untuk passing
